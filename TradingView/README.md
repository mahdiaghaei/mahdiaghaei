...
- **Session timezone**: Defaults to `Exchange`, which uses the symbol timezone. Choose a fixed timezone from the list if you want the session to follow another timezone.
...
TradingView Pine Script v6 scripts cannot create the exact built-in chart type UI or access every internal feature of TradingView's native paid Volume Profile tools. This script is an overlay approximation with comparable controls and visual behavior, and it is intentionally real-time only.

//+------------------------------------------------------------------+
//|                                                MarketProfile.mq4  |
//|                       Session Market Profile indicator for MT4    |
//+------------------------------------------------------------------+
#property strict
...
string ObjectPrefix = "MP_";

//+------------------------------------------------------------------+
int OnInit()
{
   IndicatorShortName("Market Profile (Session TPO)");
                         ...
                         const int rates_total)
{
   ...
   double step = MathMax(PointsPerPriceStep * Point, Point);
                        ...
                        const int value_high_row)
{
   ...
   for(int profile_row = 0; profile_row < rows; profile_row++)
   {
      ...
      double price_bottom = NormalizeDouble(session_low + (profile_row * step), Digits);
      double price_top = NormalizeDouble(price_bottom + step, Digits);
   ...
   double poc_price = NormalizeDouble(session_low + (poc_row * step) + (step / 2.0), Digits);

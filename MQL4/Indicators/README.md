...
- Exposes inputs for session start/end, weekly profile count, bucket size, maximum rows, colors, labels, and chart-time anchoring.
...
6. Keep `UseVisibleChartTime` enabled when using Soft4FX Forex Simulator or Strategy Tester so profiles are anchored to the visible/tested chart date instead of the broker/server current date.
